...
async function run() {
  ...
  try {
    ...
    const verifyAdmin = async (req, res, next) => {
      ...
      if (!user || user?.role !== 'admin') return res.status(403).send({ message: 'AdminOnly Actions!' })
    ...
    const verifySeller = async (req, res, next) => {
      const email = req?.user?.email
      const user = await usersCollection.findOne({ email })
      if (!user || user?.role !== 'seller') return res.status(403).send({ message: 'AdminOnly Actions!' })
    ...
    app.post('/user', async (req, res) => {
      ...
      console.log('User already exists: ', !!alreadyExists)   //!! means alreadyExists true hole true false hole false dibe.+boolian value dibe
      if (!!alreadyExists) {
    ...
    app.get('/all-users', verifyToken, verifyAdmin, async (req, res) => {
      ...
      const filter = {
        email: {
          $ne: req?.user?.email   // ai email bade sob email dibe
        }
      }
    ...
    app.get('/admin-stats', verifyToken, async (req, res) => {
      // const totalUser = await usersCollection.countDocuments({ role: 'admin' }) admin koijon dibe
      ...
      const result = await ordersCollection.aggregate([
        //_id theke created bar kore arakte field create kora
        {
          $addFields: {
      ...
      // res.send({ totalUser, totalPlant, totalOrders })
      res.send({ totalUser, totalPlant, barChartData, totalOrders, totalRevenue })
    })
